...
export async function GET(request: Request) {
  try {
    ...
    const [
      [settingsRows],
      [[heroRow], [statsRows]],
      [[flowRow], [flowStepsRows]],
      [villasRows],
      [seasonsMetaRows],
      [stayPlansRows],
      [[locationRow], [accessRows]],
      [[ctaRow]],
      [settingsForCta],
      [sectionsRows],
    ] = await Promise.all([
    ...
    for (const row of settingsRows as any[]) {
      if (row.value_raw !== null && row.value_raw !== undefined) {
        settings[row.setting_key] = row.value_raw;
      } else {
        settings[row.setting_key] = { zh: row.value_zh ?? '', ja: row.value_ja ?? '', en: row.value_en ?? '' };
      }
    }

    // Hero
    const h = (heroRow as any[])[0] ?? {};
    const hero = {
    ...
    // Flow
    const fs = (flowRow as any[])[0] ?? {};
    const flow = {
      eyebrow:  { zh: fs.eyebrow_zh ?? '',  ja: fs.eyebrow_ja ?? '',  en: fs.eyebrow_en ?? '' },
      title:    { zh: fs.title_zh ?? '',    ja: fs.title_ja ?? '',    en: fs.title_en ?? '' },
      subtitle: { zh: fs.subtitle_zh ?? '', ja: fs.subtitle_ja ?? '', en: fs.subtitle_en ?? '' },
    ...
    return NextResponse.json(
      { settings, hero, flow, villas, seasonsMeta, plans, location, cta, sections },
      { headers: NO_CACHE },
    );
  } catch (err) {
    console.error('[home/all GET]', err);
